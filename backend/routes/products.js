...
router.get('/', function (req, res) {       // Sending Page Query Parameter is mandatory http://localhost:3636/api/products?page=1
    ...
    const limit = (req.query.limit !== undefined && req.query.limit !== 0) ? req.query.limit : 47;   // set limit of items per page
    let startValue;
    let endValue;
    if (page > 0) {
...
router.get('/category/:catName', (req, res) => { // Sending Page Query Parameter is mandatory http://localhost:3636/api/products/category/categoryName?page=1
    ...
    const limit = (req.query.limit !== undefined && req.query.limit !== 0) ? req.query.limit : 25;   // set limit of items per page
    let startValue;
    let endValue;
    if (page > 0) {
        startValue = (page * limit) - limit;      // 0, 10, 20, 30
    ...
    database.table('products as p')
        .join([
            {
                table: "categories as c",
                on: `c.category_id = p.category_id WHERE c.title LIKE '%${cat_title}%'`
            }
        ])
  .withFields(['c.title as category',
      'p.name',
